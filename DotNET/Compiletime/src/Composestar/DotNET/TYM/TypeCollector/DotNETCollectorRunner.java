...
{    
	...
	public void run(CommonResources resources) throws ModuleException
	{
		 ...
		 * Therefore:
		 * 1) embedded code that is fully programedd, like the Sound concern of Pacman, must be ignored in this part of code;
		...
		while (concernIt.hasNext())
		{
			...
			{
				throw new ModuleException("CollectorRunner: Can only handle concerns with source file implementations or direct class links.", MODULE_NAME);
			}

			// transform source name into assembly name blaat.java --> blaat.dll
			if( !typeMap.containsKey( className ) ) {
				throw new ModuleException("Implementation: " + className + " for concern: " + concern.getName() + " not found!", MODULE_NAME);
