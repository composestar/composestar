...
public class DotNETTypeResolver extends TypeResolver
{
	...
	protected void resolveDotNETType(DotNETType unit, UnitRegister register)
	{
		...
		if (unit.getUnderlyingSystemType() != null)
		{
			DotNETType type = (DotNETType) register.getType(unit.getUnderlyingSystemType());
			if (type == null)
			{
				logger.info(String.format("Unable to underlying system type \"%s\" for type \"%s\"", unit
						.getUnderlyingSystemType(), unit.getFullName()));
			}
			else
			{
				unit.setUnderlyingSystemType(type);
			}
		}
	}
}
