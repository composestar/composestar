...
/*
 * $Id: FontFactoryImp.cs,v 1.5 2006/08/28 20:20:50 psoares33 Exp $
 * $Name:  $
 *
        ...
        public virtual int RegisterDirectory(String dir) {
            ...
            try {
                ...
                for (int k = 0; k < files.Length; ++k) {
                    try {
                        String name = files[k].ToLower(CultureInfo.InvariantCulture);
                        if (name.EndsWith(".ttf") || name.EndsWith(".otf") || name.EndsWith(".afm") || name.EndsWith(".pfm") || name.EndsWith(".ttc")) {
                            Register(Path.GetFullPath(files[k]), null);
                            ++count;
                        }
                    }
                    catch  {
