    ...
    public abstract class RtfField : Chunk, iTextSharp.text.rtf.IRtfBasicElement {
        ...
        */
        private byte[] WriteFieldBegin() {
            ...
            if (fieldDirty) {
                result.Write(FIELD_DIRTY, 0, FIELD_DIRTY.Length);
            }
            if (fieldEdit) {
                result.Write(FIELD_EDIT, 0, FIELD_EDIT.Length);
            }
            if (fieldLocked) {
                result.Write(FIELD_LOCKED, 0, FIELD_LOCKED.Length);
            }
            if (fieldPrivate) {
                result.Write(FIELD_PRIVATE, 0, FIELD_PRIVATE.Length);
            }
        ...
        */
        public virtual byte[] Write() {
            ...
            try {
                ...
                result.Write(t = WriteFieldBegin(), 0, t.Length);
                result.Write(t = WriteFieldInstBegin(), 0, t.Length);
                result.Write(t = WriteFieldInstContent(), 0, t.Length);
                result.Write(t = WriteFieldInstEnd(), 0, t.Length);
                result.Write(t = WriteFieldResultBegin(), 0, t.Length);
                result.Write(t = WriteFieldResultContent(), 0, t.Length);
                result.Write(t = WriteFieldResultEnd(), 0, t.Length);
                result.Write(t = WriteFieldEnd(), 0, t.Length);
