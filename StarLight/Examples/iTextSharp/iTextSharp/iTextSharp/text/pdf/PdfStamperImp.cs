    ...
    public class PdfStamperImp : PdfWriter {
        ...
        */
        ...
        internal void Close(Hashtable moreInfo) {
            ...
            AddFileAttachments();
            if (openAction != null) {
                reader.Catalog.Put(PdfName.OPENACTION, openAction);
            }
            ...
            if (moreInfo != null) {
                foreach (string key in moreInfo.Keys) {
                    PdfName keyName = new PdfName(key);
                    String value = (String)moreInfo[key];
                    if (value == null)
        ...
        internal void AlterContents() {
            foreach (PageStamp ps in pagesToContent.Values) {
                ...
                if (ps.over != null) {
                    ...
                    out_p.Append(PdfContents.SAVESTATE);
                    ApplyRotation(pageN, out_p);
                    out_p.Append(ps.over.InternalBuffer);
                    out_p.Append(PdfContents.RESTORESTATE);
        ...
        internal void FlatFields() {
            ...
            foreach (String name in fields.Keys) {
                if (partialFlattening.Count != 0 && !partialFlattening.ContainsKey(name))
                    continue;
                AcroFields.Item item = (AcroFields.Item)fields[name];
    ...
    //        reader.GetCatalog().Remove(PdfName.ACROFORM);
        ...
        */
        ...
        internal void SetJavaScript() {
            ArrayList djs = pdf.DocumentJavaScript;
            if (djs.Count == 0)
                return;
            PdfDictionary catalog = reader.Catalog;
            ...
            Hashtable maptree = new Hashtable();
            for (int k = 0; k < djs.Count; ++k) {
                s = k.ToString();
        ...
        void AddFileAttachments() {
            ...
            foreach (String name in fs.Keys) {
                int k = 0;
                String nn = name;
        ...
        internal void SetOutlines() {
            ...
            if (newBookmarks.Count == 0)
                return;
            namedAsNames = (reader.Catalog.Get(PdfName.DESTS) != null);
            ...
            reader.Catalog.Put(PdfName.OUTLINES, topRef);
            MarkUsed(reader.Catalog);
        }
        
        internal ArrayList Outlines {
            set {
                newBookmarks = value;
            }
        }
