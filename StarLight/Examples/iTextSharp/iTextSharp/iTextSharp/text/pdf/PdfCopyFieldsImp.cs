    ...
    */
    ...
    //            AddToBody(obj, refo);
        ...
        protected PdfArray BranchForm(Hashtable level, PdfIndirectReference parent, String fname) {
            PdfArray arr = new PdfArray();
            foreach (String name in level.Keys) {
                Object obj = level[name] ;
        ...
        protected void CloseIt() {
            ...
            foreach (PdfReader reader in readers2intrefs.Keys) {
                try {
                    file = reader.SafeFile;
                    file.ReOpen();
                    IntHashtable t = (IntHashtable)readers2intrefs[reader];
        ...
        internal void MergeWithMaster(Hashtable fd) {
            foreach (String name in fd.Keys) {
                MergeField(name, (AcroFields.Item)fd[name]);
            }
        }
