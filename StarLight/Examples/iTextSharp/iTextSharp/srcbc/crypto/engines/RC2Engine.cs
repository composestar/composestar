	...
	public class RC2Engine : BlockCipher
	{
		...
		*/
			...
			int outOff)
		{
			if (workingKey == null)
			{
				throw new Exception("RC2 engine not initialised");
			}
		...
		*/
			...
			int     outOff)
		{
			...
			x76 = ((inBytes[inOff + 7] & 0xff) << 8) + (inBytes[inOff + 6] & 0xff);
			x54 = ((inBytes[inOff + 5] & 0xff) << 8) + (inBytes[inOff + 4] & 0xff);
			x32 = ((inBytes[inOff + 3] & 0xff) << 8) + (inBytes[inOff + 2] & 0xff);
			x10 = ((inBytes[inOff + 1] & 0xff) << 8) + (inBytes[inOff + 0] & 0xff);
