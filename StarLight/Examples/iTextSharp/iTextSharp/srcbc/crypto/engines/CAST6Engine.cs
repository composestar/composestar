...
namespace org.bouncycastle.crypto.engines
{
	...
	public sealed class CAST6Engine : CAST5Engine
	{
		...
		new public static readonly int    BLOCK_SIZE = 16;  // bytes = 128 bits

		/*
		* Put the round and mask keys into an array.
		* Kr0[i] => _Kr[i*4 + 0]
		*/
		new public int []_Kr = new int[ROUNDS*4]; // the rotating round key(s)
		new public int []_Km = new int[ROUNDS*4]; // the masking round key(s)

		/*
		* Key setup
		*/
		public int []_Tr = new int[24 * 8];
		public int []_Tm = new int[24 * 8];
		...
		protected override void setKey(byte[] key)
		{
			...
			*/
			for (int i=0; i< 24; i++)
			{
				...
				{
					_Tm[i*8 + j] = Cm;
					Cm = (Cm + Mm) & unchecked((int) 0xffffffff);	// mod 2^32;
