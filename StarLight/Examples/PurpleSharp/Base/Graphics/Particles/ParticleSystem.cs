...
namespace Purple.Graphics.Particles {
  ...
  public class ParticleSystem : ParticleSystemBase {
    ...
    public ParticleSystem( int size ) : base( ) {
      ...
      indexStream = IndexStream.FromQuads(size-1);

      // load shaders from the resource files
      using (System.IO.Stream fxStream = Purple.IO.ResourceFileSystem.Instance.Open("Purple/Graphics/Particles/Particle.fx")) {
