...
namespace SourceGrid.Cells.Virtual
{
	...
	public class ColumnHeader : CellVirtual
	{
		...
		/// <summary>
		/// Gets or sets if enable the selection on the column when clicking this header, using a ColumnSelector. Default is false.
		/// </summary>
		public bool ColumnSelectorEnabled
		{
			get{return FindController(typeof(Controllers.ColumnSelector)) == Controllers.ColumnSelector.Default;}
			set
			{
				if (value)
					AddController(Controllers.ColumnSelector.Default);
				else
					RemoveController(Controllers.ColumnSelector.Default);
			}
		}

		/// <summary>
		/// Gets or sets if enable the focus on the column when clicking this header, using a ColumnFocus controller. Default is true.
		/// </summary>
		public bool ColumnFocusEnabled
		{
			get{return FindController(typeof(Controllers.ColumnFocus)) == Controllers.ColumnFocus.Default;}
			set
			{
				if (value)
					AddController(Controllers.ColumnFocus.Default);
				else
					RemoveController(Controllers.ColumnFocus.Default);
			}
		}
	}
}
...
namespace SourceGrid.Cells
{
	...
	public class ColumnHeader : Cell
	{
		...
		/// <summary>
		/// Gets or sets if enable the selection on the column when clicking this header, using a ColumnSelector controller. Default is false.
		/// </summary>
		public bool ColumnSelectorEnabled
		{
			get{return FindController(typeof(Controllers.ColumnSelector)) == Controllers.ColumnSelector.Default;}
