...
namespace Composestar.StarLight.VisualStudio.LanguageServices {
    ...
    /// <summary>
    /// This interface defines the service that finds ComposeStar files inside a hierarchy
    /// and builds the informations to expose to the class view or object browser.
    /// </summary>
    [Guid(ComposeStarConstants.libraryManagerServiceGuidString)]
    public interface IComposeStarLibraryManager {
    ...
    /// navigation tools (class view or object browser) from the ComposeStar files inside a
    /// hierarchy.
    /// </summary>
    [Guid(ComposeStarConstants.libraryManagerGuidString)]
    internal class ComposeStarLibraryManager : IComposeStarLibraryManager, IVsRunningDocTableEvents, IDisposable {

        /// <summary>
        /// Class storing the data about a parsing task on a concern.
        /// A module in ComposeStar is a source file, so here we use the file name to
        /// identify it.
        /// </summary>
        ...
        private Dictionary<ConcernId, LibraryNode> files;
        ...
        #region IComposeStarLibraryManager
        ...
        public void UnregisterHierarchy(IVsHierarchy hierarchy) {
            ...
            lock (files) {
                ConcernId[] keys = new ConcernId[files.Keys.Count];
                files.Keys.CopyTo(keys, 0);
                foreach (ConcernId id in keys) {
        ...
        private void ParseThread() {
            ...
            while (true) {
                ...
                CreateModuleTree(module, module, "", task.ConcernId);
                if (null != task.ConcernId) {
                    LibraryNode previousItem = null;
                    lock (files) {
                        if (files.TryGetValue(task.ConcernId, out previousItem)) {
                            files.Remove(task.ConcernId);
                        }
                    }
                ...
                if (null != task.ConcernId) {
                    lock(files) {
                        files.Add(task.ConcernId, module);
                    }
                }
            }
        }

        private void CreateModuleTree(LibraryNode root, LibraryNode current, string namePrefix, ConcernId concernId)
        {
          root.AddNode(new ComposeStarLibraryNode(namePrefix, concernId.Hierarchy, concernId.ItemID));
            //foreach (ScopeNode subItem in scope.NestedScopes) {
            //    ComposeStarLibraryNode newNode = new ComposeStarLibraryNode(subItem, namePrefix, moduleId.Hierarchy, moduleId.ItemID);
        ...
        private void CreateParseRequest(string file, string text, ConcernId id) {
        ...
        private void OnNewFile(object sender, HierarchyEventArgs args) {
            ...
            CreateParseRequest(args.CanonicalName, fileText, new ConcernId(hierarchy, args.ItemID));
        }
        ...
        private void OnDeleteFile(object sender, HierarchyEventArgs args) {
            ...
            ConcernId id = new ConcernId(hierarchy, args.ItemID);
        ...
        public int OnBeforeDocumentWindowShow(uint docCookie, int fFirstShow, IVsWindowFrame pFrame)
        {
            ...
            if (null != rdt) {
                ...
                try {
                    ...
                    if (0 != string.Compare(extension, ComposeStarConstants.composeStarFileExtension, StringComparison.OrdinalIgnoreCase)) {
                        return VSConstants.S_OK;
                    }

                    // Create the module id for this document.
                    ConcernId docId = new ConcernId(hierarchy, itemId);
