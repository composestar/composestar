			...
			foreach (MSBuild.BuildItem item in this.BuildProject.EvaluatedItems)
			{
				if (String.Compare(item.Name, ProjectFileConstants.SubProject, StringComparison.OrdinalIgnoreCase) == 0)
				{
					...
					else
					{
						...
						String template = Path.GetExtension(GetProjectTemplatePath(null));
						if (String.Compare(template, ".vstemplate", StringComparison.OrdinalIgnoreCase) == 0
							|| String.Compare(template, ".vsz", StringComparison.OrdinalIgnoreCase) == 0)
						{
