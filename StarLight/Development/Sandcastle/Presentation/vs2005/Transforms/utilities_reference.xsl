...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.1"
        ...
        <xsl:for-each select="parameter">
          ...
          <dl paramName="{$paramName}">
            <dt>
              ...
              <xsl:text>&#x20;(</xsl:text>
              <xsl:call-template name="showQualifiedTypeLink"/>
              <xsl:text>)</xsl:text>
            </dt>
            <dd>
  ...
  <xsl:template name="showQualifiedTypeLink">
    <xsl:choose>
      <!-- type is a type parameter of the current topic; just show the name -->
      <xsl:when test="templateparameter[@declaringmember=$key]">
        <xsl:value-of select="@name"/>
      </xsl:when>
      <!-- type is a type parameter declared on another API -->
      <xsl:when test="templateparameter">
        <include item="typeLinkToTypeParameter">
          <parameter>
            <xsl:value-of select="templateparameter/@name"/>
          </parameter>
          <parameter>
            <xsl:call-template name="createReferenceLink">
              <xsl:with-param name="id" select="templateparameter/@declaringMember"/>
      ...
      <!-- type is a more or less normal managed type -->
      <xsl:otherwise>
        <xsl:for-each select="*">
  ...
  <xsl:template name="typeList">
    ...
    <xsl:variable name="visibilityMode" select="memberdata/@visibility"/>
    <xsl:variable name="subgroupId" select="apidata/@subgroup"/>
   
    <xsl:if test="$subgroup = $subgroupId and $visibility = $visibilityMode">
...
-->
  ...
  <xsl:template match="elements" mode="namespace">
    ...
    <xsl:if test="element/apidata/@subgroup = 'class'">
      <xsl:call-template name="section">
        <xsl:with-param name="toggleSwitch" select="'class'"/>
        <xsl:with-param name="title">
    ...
    <xsl:if test="element/apidata/@subgroup = 'structure'">
      <xsl:call-template name="section">
        <xsl:with-param name="toggleSwitch" select="'structure'"/>
        <xsl:with-param name="title">
    ...
    <xsl:if test="element/apidata/@subgroup = 'interface'">
      <xsl:call-template name="section">
        <xsl:with-param name="toggleSwitch" select="'interface'"/>
        <xsl:with-param name="title">
    ...
    <xsl:if test="element/apidata/@subgroup = 'delegate'">
      <xsl:call-template name="section">
        <xsl:with-param name="toggleSwitch" select="'delegate'"/>
        <xsl:with-param name="title">
    ...
    <xsl:if test="element/apidata/@subgroup = 'enumeration'">
      <xsl:call-template name="section">
        <xsl:with-param name="toggleSwitch" select="'enumeration'"/>
        <xsl:with-param name="title">
  ...
  <xsl:template match="elements" mode="type">
    ...
    <xsl:if test="element/apidata[@subgroup='method']">
      <xsl:call-template name="section">
        <xsl:with-param name="toggleSwitch" select="'Method'" />
        <xsl:with-param name="title">
          <include item="Methods" />
        </xsl:with-param>
        <xsl:with-param name="content">
          <xsl:call-template name="tableList">
    ...
    <xsl:if test="element/apidata[@subgroup='constructor']">
      <xsl:call-template name="section">
        <xsl:with-param name="toggleSwitch" select="'Constructor'" />
        <xsl:with-param name="title">
          <include item="Constructors" />
        </xsl:with-param>
        <xsl:with-param name="content">
          <xsl:call-template name="tableList">
    ...
    <xsl:if test="element/apidata[@subgroup='property']">
      <xsl:call-template name="section">
        <xsl:with-param name="toggleSwitch" select="'Property'" />
        <xsl:with-param name="title">
   ...
   <xsl:if test="element/apidata[@subgroup='event']">
     <xsl:call-template name="section">
       <xsl:with-param name="toggleSwitch" select="'Event'" />
       <xsl:with-param name="title">
         <include item="Events" />
       </xsl:with-param>
       <xsl:with-param name="content">
