 ...
 * */
package composestarEclipsePlugin.makefile;
...
import composestarEclipsePlugin.*;
import composestarEclipsePlugin.buildConfiguration.*;
import composestarEclipsePlugin.dialogs.StandardSettings;
...
public class MakefileCreator {
	...
	public MakefileCreator(String projectPath, IProjectDescription projectDescription,
								HashSet sources, HashSet concerns, String customFilters){
			...
			String[] eclipseInstallation= org.eclipse.core.runtime.Platform.getInstallLocation().getURL().getPath().split("/");//toCharArray(); //toString
	...
	private void copyMessageFiles(BufferedWriter bw){
		try{
			...
			//copy /Y c:\local\Johan\Eclipse\eclipse\plugins\ComposestarEclipsePlugin\message.* H:\MingwTest\EclipsePlugin\EncryptionExample\bin\
			bw.write("\t$(fileCopy) "+eclipseInstallationOS+"plugins"+ java.io.File.separatorChar +"ComposestarEclipsePlugin"+ java.io.File.separatorChar +"message.* "+ basePath + "\n");
			bw.write("\n");
		}
	...
	private void preprocess(BufferedWriter bw)
	{
		...
		{	bw.write("preprocess:\n");
			...
			while(i.hasNext()){
				...
				bw.write("\tgcc -dD -E" + define()+" " + element.lastSegment()+" -o "+element.lastSegment()+"cc"+ "\n"); //((IPath)i.next()).toOSString() ((IPath)i).lastSegment()) ||outputPath+ java.io.File.separatorChar +
			}
	...
	private void weave(BufferedWriter bw)
	{
			try{
			/**bw.write("weave:\n");
			String path=eclipseInstallationOS.replace("\"","")+"plugins"+java.io.File.separatorChar+"ComposestarEclipsePlugin"+java.io.File.separatorChar+"binaries"+java.io.File.separatorChar;//"C:/local/Johan/EclipsePlugin/ComposestarEclipsePlugin/";
			String jvmOptions = "";
			String antlr= path+"antlr.jar$(SEP)";
			String prolog= path+ "prolog"+java.io.File.separatorChar+"prolog.jar$(SEP)"; //org.eclipse.core.runtime.Platform.getInstallLocation().getURL().getPath().toString()+"plugins"+java.io.File.separatorChar+"ComposestarEclipsePlugin"+java.io.File.separatorChar+"bin"+java.io.File.separatorChar+"prolog.jar;";
			...
			String command = "java.exe " + jvmOptions + " -cp \"" +antlr+prolog+cC+cCORE+customFilters + "\" " + mainClass + " " + "\"" +projectPath+"BuildConfiguration.xml" + "\"";//  ss.getClassPathString()+ "C:/local/Johan/ComposestarCVS/composestarcore/src/;C:/local/Johan/ComposestarCVS/composestarc/src/"
			**/
			bw.write("weave:\n");
			String path=eclipseInstallationOS.replace("\"","")+"plugins"+java.io.File.separatorChar+"ComposestarEclipsePlugin"+java.io.File.separatorChar+"binaries"+java.io.File.separatorChar;//"C:/local/Johan/EclipsePlugin/ComposestarEclipsePlugin/";
			String jvmOptions = "";
			String xerces= path+"xerces-2_6_0-xml-apis.jar$(SEP)";
			String xercesImpl= path+"xerces-2_6_0-xercesImpl.jar$(SEP)";
			String jgraph= path+"jgraph.jar$(SEP)";
			String castor= path+"castor-0_9_5_2-xml.jar$(SEP)";
			String groove= path+"groove-1_2_0.jar$(SEP)";
			String antlr= path+"antlr.jar$(SEP)";
			String prolog= path+ "prolog"+java.io.File.separatorChar+"prolog.jar$(SEP)"; //org.eclipse.core.runtime.Platform.getInstallLocation().getURL().getPath().toString()+"plugins"+java.io.File.separatorChar+"ComposestarEclipsePlugin"+java.io.File.separatorChar+"bin"+java.io.File.separatorChar+"prolog.jar;";
			String cC= path+ "ComposestarC.jar$(SEP)";
			String cCORE= path+"ComposestarCORE.jar$(SEP)";
			String mainClass = "Composestar.C.MASTER.CMaster";
			customFilters=customFilters.replaceAll("\n","");
			String command = "java.exe " + jvmOptions + " -cp \"" +xerces+xercesImpl+jgraph+castor+groove+antlr+prolog+cC+cCORE+customFilters + "\" " + mainClass + " " + "\"" +projectPath+"BuildConfiguration.xml" + "\"";//  ss.getClassPathString()+ "C:/local/Johan/ComposestarCVS/composestarcore/src/;C:/local/Johan/ComposestarCVS/composestarc/src/"
	...
	{	
		try{
			/***************************************
			 * Copy message.c en message.h naar output dir
			 * Doe dit ook met de concerns
			 * Roep dan gcc met preprocessed files en deze files aan
			 */
			...
			bw.write("\t gcc -o"+ outputPath.replace(""+java.io.File.separatorChar,"/")+projectDescription.getName()+".exe" +" "+command+" "+ outputPath.replace(""+java.io.File.separatorChar,"/")+"message.c \n"); 
		}
		catch(IOException io) {
			Debug.instance().Log("Error while creating MakeFile: ",Debug.MSG_ERROR);
		}
	...
	private void build(BufferedWriter bw)
	{
		try
		{
			bw.write("build: copyMessageFromPlugin preprocess weave copymessage copyhfiles changeextensions compile clean run\n");
		}
		catch(IOException io) {
			Debug.instance().Log("Error while creating MakeFile: ",Debug.MSG_ERROR);
		}
	...
	private void buildWithoutRun(BufferedWriter bw)
	{
		try
		{
			bw.write("buildWithoutRun: copyMessageFromPlugin preprocess weave copymessage copyhfiles changeextensions compile clean \n");
		}
		catch(IOException io) {
			Debug.instance().Log("Error while creating MakeFile: ",Debug.MSG_ERROR);
		}
	...
	private void buildWithoutConcerns(BufferedWriter bw)
	{
		try
		{
			bw.write("buildWithOutConcerns: copyMessageFromPlugin copymessage copycfiles copyhfiles compile clean run\n");
		}
		catch(IOException io) {
			Debug.instance().Log("Error while creating MakeFile: ",Debug.MSG_ERROR);
		}
	...
	private void buildKeepIntermediateFiles(BufferedWriter bw)
	{
		try
		{
			bw.write("buildKeepIntermediateFiles: copyMessageFromPlugin preprocess weave copymessage copyhfiles changeextensions compile run\n");
		}
		catch(IOException io) {
			Debug.instance().Log("Error while creating MakeFile: ",Debug.MSG_ERROR);
		}
	...
	private void settings(BufferedWriter bw){
		try{
			Debug.instance().Log(org.eclipse.core.runtime.Platform.getInstallLocation().getURL().getPath().toString()+"plugins"+java.io.File.separatorChar+"ComposestarEclipsePlugin");
	...
	private void osSettings(BufferedWriter bw){
		try{
			...
			bw.write("\tSEP=:\n");
			bw.write("\tMAKE=make\n");
			bw.write("\tRECURSE_MAKE_RULE=if ["+ '"'+ "$(SUBDIRS)"+ '"'+ "!=\"\" ]; then for a in $(SUBDIRS); do cd $$a && $(MAKE) && cd ..; done fi\n");
			bw.write("\tRECURSE_CLEAN_RULE=if [ \"$(SUBDIRS)\" != \"\" ]; then for a in $(SUBDIRS); do cd $$a && $(MAKE) clean && cd ..; done fi\n");
			bw.write("\tDEL=rm -f\n");
			...
			bw.write("\tRECURSE_MAKE_RULE=if not \"$(SUBDIRS)\" == \"\" for %%a in ( $(SUBDIRS) ) do cd %%a && $(MAKE) && cd..; done fi\n");
			bw.write("\tRECURSE_CLEAN_RULE=if not \"$(SUBDIRS)\" == \"\" for %%a in ( $(SUBDIRS) ) do cd %%a && $(MAKE) clean && cd..; done fi\n");
			bw.write("\tDEL=del\n");
			bw.write("\tMKDIR=md\n");
