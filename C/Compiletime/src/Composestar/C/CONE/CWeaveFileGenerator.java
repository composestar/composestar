...
/**
 * This class generates the aspects for weavec, An aspect can add header files
 * or add filter advice to super imposed functions
 * 
...
public class CWeaveFileGenerator
{
	...
	public void createAspects()
	{
		...
		while (it.hasNext())
		{
			...
			{
				/**
				 * Dit levert de namen van de filtermodules die op dit concern
				 * werken op*
				 */
				if (c.getDynObject("SingleOrder") != null && c.getDynObject("SingleOrder") instanceof FilterModuleOrder)
				{
					...
					for (int i = 0; i < order.size(); i++)
					{
						...
						Debug.out(Debug.MODE_INFORMATION, "cone", "FQN: "
								...
								+ " Concern : "
								+ c.getName()
								+ "has Filtermodule with name: "
								+ fm.getName()
								+ " Number of [m,i,filtertype,internals] : ["
								+ ((MatchingPart) ((Filter) fm.inputFilters.elementAt(0)).getFilterElement(0)
										.getMatchingPattern().getMatchingParts().elementAt(0)).getSelector().getName()
								+ "," + fm.inputFilters.size() + ","
						...
						if (!InnerDispatcher.isDefaultDispatch(fm))
						{
							...
							while (inputfilters.hasNext())
							{
								...
								for (int x = 0; x < numberOfFilterElements; x++)
								{
									...
									if (c.getPlatformRepresentation() instanceof CFile)
									{
										...
										if (!((MatchingPart) (filter.getFilterElement(x).getMatchingPattern()
												.getMatchingParts().elementAt(0))).getSelector().getName().equals("*"))
										{
											if ((file.getMethodInfo(((MatchingPart) (filter.getFilterElement(x)
													.getMatchingPattern().getMatchingParts().elementAt(0)))
													.getSelector().getName())) != null)
											{
												methods.put((file.getMethodInfo(((MatchingPart) (filter
														.getFilterElement(x).getMatchingPattern().getMatchingParts()
														.elementAt(0))).getSelector().getName())), file);
											}
										}
										else
										{
										 ...
										 * functions of a defined concern in
										 * CConcern.xml we look if there is a
										 * file without any functions if this is
										 * the case and the signature of the
										 * concern does has a function with the
										 * name of the method in the filter then
										 * this is the method where is
										 * superimposed on.
										 */
									...
									{
										addConditionHeader((filter.getFilterElement(x).getConditionPart())
												.getDescriptionFileName(), method, file);
									}
									...
									while (methodsIter.hasNext())
									{
										...
										if (beforeAdvice != null)
										{
											...
											beforeAA
													.setId(filter.getName() + x + method.Name + methodOffset + "before");// .getFilterElement(0).getMatchingPattern().getMatchingParts().elementAt(0)).getSelector().getName());//.getFilterElement(0).getMatchingPattern().getMatchingParts().elementAt(0)).getSelector().getName());
										...
										// addSubstitutionTargetHeader(filter,
										// x);
									}
								}
							...
							while (outputfilters.hasNext())
							{
								...
								for (int x = 0; x < numberOfFilterElements; x++)
								{
									...
									{
										addConditionHeader(
												(filter.getFilterElement(x).getConditionPart()).descriptionFileName,
												method, file);// updated
										// 31-10
										// ConditionLiteral
									}
			...
			String adviceType)
	{
		...
		else if (filter.getFilterType().getType().equals(FilterTypeNames.CUSTOM))
		{
			...
			else
			{
				try
				{
					...
					{
						code = filterSemantic.getBeforeAdvice();
						if (filterSemantic.redirectMessage())
						{
							addSubstitutionTargetHeader(filter, filterelem);
						}
						if (filterSemantic.needsHeaderFiles())
						{
							addHeader(filter, filterelem, filterSemantic.headerFile());
						}
					}
					else if (adviceType.equals("after") && filterSemantic.afterAdvice())
					{
		...
		{
			Debug.out(Debug.MODE_ERROR, "CONE", "Filter type not found: " + filter.getFilterType().getType());
		}
		Advice advice = new Advice();
		advice.setCode(code);
		int ttype = 0;
	...
	public void addAssertHeader()
	{
		if (!aspects.containsKey(file + "Assert"))
		{
			...
			int type = GeneralUtils.HEADER;
			Functions functions = new Functions();
			functions.setFile(file.getFullName() + ".c");
			functions.setData(file.getFullName() + ".c");
	...
	public void addConditionHeader(String condition, CMethodInfo methodInfo, CFile cfile)
	{
		...
		while (it.hasNext())
		{
			...
			if (!(c instanceof CpsConcern) && c.getPlatformRepresentation() instanceof CFile)
			{
				...
				if (conditionFile.getMethodInfo(condition) != null)
				{
					break;
				}
				else
		...
		aspect.setId(conditionFile + cfile.getFullName() + "CH");
		AdviceApplication aa = new AdviceApplication();

		aa.setId(conditionFile + cfile.getFullName() + "CH");
		aa.setType(GeneralUtils.BEFORE);
		Pointcut pointcut = new Pointcut(conditionFile + cfile.getFullName() + "CH");
		int type = GeneralUtils.HEADER;
		Functions functions = new Functions();
		functions.setFile(cfile.getFullName() + ".c");
		functions.setData(cfile.getFullName() + ".c");
		...
		Advice advice = new Advice();
		int ttype = GeneralUtils.HEADER_INTRODUCTION;
		advice.setId(conditionFile + cfile.getFullName() + "CH");
		advice.setType(ttype);
		advice.setPriority(1);
		advice.setCode("#include \"" + conditionFile.getFullName() + ".h\"");
		aspect.addAdvice(advice);
		pointcut.addAdviceApplication(aa);
		pointcut.setParent(aspect);
		pointcut.addFunctions(functions);
		aspect.addPointcut(pointcut);
		aspects.put(conditionFile + cfile.getFullName() + "CH", aspect);
	...
	public void addHeader(Filter filter, int filterelem, String filename)
	{
		...
		String matchingFile = file.getFullName();
	...
	public void addSubstitutionTargetHeader(Filter filter, int filterelem)
	{
		...
		Debug.out(Debug.MODE_INFORMATION, "cone", "Aspect:" + matchingFunction + substitutionFile.getUnitName()
				+ "STH superimposes:" + substitutionFile.getUnitName());
