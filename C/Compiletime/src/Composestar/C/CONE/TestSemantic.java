...
public class TestSemantic extends Semantic
{
	...
	public String getBeforeAdvice()
	{
		...
		{
			advice = reifiedMessage + " if(" + disableOperatorCode() + conditionCode() + ") {" + substitutionFunction
					+ "(&msg);}\n";
		}
		return advice;
	}
	...
	public String getAfterAdvice()
	{
		if (methodAnnotation() != null)
		{
			...
			advice = "printf(\"methodAnnotation: type=" + annotationType(methodAnnotation()) + " value="
					+ annotationvalue + "\");";
		}
		if (fileAnnotation() != null)
		{
			advice += "printf(\"FileAnnotation: type=" + annotationType(fileAnnotation()) + " value="
					+ annotationValue(fileAnnotation()) + "\");";
		}
		for (int i = 0; i < numberOfParametersMF(); i++)
		{
			if (parameterAnnotation(i) != null)
			{
				advice += "printf(\"parameterAnnotationNumber:" + i + ": type="
						+ annotationType(parameterAnnotation(i)) + " value=" + annotationValue(parameterAnnotation(i))
						+ "\");";
			}
		}
		return advice;
