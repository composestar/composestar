...
public abstract class Semantic {
	...
	protected CMethodInfo method;
	 ...
	 /**
	   * @return Code that creates the reified message struct 
	...
	protected String reifiedMessageCode(){
		...
		//CMethodInfo method =(CMethodInfo)file.getMethodInfo(matchingFunction());
		if(method!=null){
	...
	protected String conditionCode(){
		...
		if(filter.getFilterElement(elementNumber).getConditionPart()instanceof ConditionLiteral){
			conditionPart=((ConditionLiteral)filter.getFilterElement(elementNumber).getConditionPart()).getName()+"()";//.getCondition()
		}
		return conditionPart;
	}
	
	/**
	   * @return ! if the operator used in the conditionexpression of the filter is ~>
	   */
	...
	protected String parameterName(int index){
		...
		if(method!=null){
			List params =method.getParameters();
			if(params.size()<=index) return "Error";
	...
	/**
	   * @return The type of the annotation of the parameter of the matching function 
	   * @param offset of the parameter from the function
	   */
	protected CAnnotation parameterAnnotation(int index){
		...
		if(method!=null){
			...
			/**TODO here is hard coded that we need to pick the first/ and only possible annotation
			 * However when annotations are extended this can give troubles
			 */
	...
	protected String annotationValue(CAnnotation anno){
		return anno.value.substring(1,anno.value.length()-1);
	}
	...
	protected String parameters(){
		String parameters= "";
		if(numberOfParametersMF()>0) parameters+=parameterName(0);
	...
	protected String parameterTypeSF(int index){
		String parameterType= "";
		CParameterInfo parameter=null;
		CMethodInfo method =(CMethodInfo)file.getMethodInfo(substitutionFunction());
		if(method!=null){
			List params =method.getParameters();
			if(params.size()<=index) return "Error";
			parameterType=((CParameterInfo)params.get(index)).getParameterTypeString();
	...
	protected String substitutionFunction(){
		return ((SubstitutionPart)(filter.getFilterElement(elementNumber).getMatchingPattern(0).getSubstitutionParts().elementAt(0))).getSelector().getName();
	}
	...
	/**
	   * @return true if the filter redirects a message to another
	   * function
	   */
	public abstract boolean redirectMessage();
	
	/**
	   * @return returns a name of a header file (including .h) as a 
	   * string that contains a header where function or global definitions
	   * of elements used in the advice are specified
	   */
	public abstract String headerFile();
	
	/**
	   * @return true if there are other headers needed
