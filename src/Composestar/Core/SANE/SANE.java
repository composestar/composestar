//Source file: D:\\Lodewijk\\software\\ComposeStar\\composestar\\src\\Composestar\\core\\SANE\\SANE.java
...
public class SANE implements CTCommonModule {
   ...
   /**
    * @roseuid 404C4B660156
    */
   public SANE() {}

   /**
    * @param entityIt iterates over all instances of CpsConcern in the iterator (i.e. repository),
    *                 and creates and attaches an SIinfo instance, then calls SIInfo.resolveSI().
    * @param entityIt
    * @roseuid 4048FC9C025F
    */
   ...
   /**
    * generate all possible sets of bindings (each represented by FilterModSIinfo)
    *
    * @param concern
    * @roseuid 40597FA9001D
    */
   protected static void bindFilterModules(CpsConcern concern)
      throws ModuleException {
      if ( concern.getSuperImposition() != null ) {
         Iterator fmBindingIter=
         ...
         while (fmBindingIter.hasNext()) {
            ...
            if (selConcerns == null) {
               throw (
                  new ModuleException("Selector has not been interpreted!"));
            }
            	...
            	if (aRef instanceof ConcernReference)
            	{
	               ...
	               if (siConcern != null) //TODO: now exception generated!
	                  ...
	                  try {
	                     ...
	                     throw new ModuleException("[SANE] Problem while doing SIInfo::addFMsAt");
	                  }
	               } else {
	                  System.out.println(
	                     "SANE> encountered null Concern in the interpretation of"
	                        + selRef.getName());
	               }
            	}
            	else
            		throw new ModuleException("Can superimpose filtermodules only on Concerns/Classes;\n" +
            				                  "make sure selector " + selRef.getQualifiedName() + " selects only elements of those types", "SANE");
            } // for
         } //for
      } // if !=null
   } //bindFilterModules()
   ...
   /**
    * generate all possible sets of bindings (each represented by CondtionsSIinfo)
    *
    * @param concern
    * @roseuid 4059814001B2
    */
   protected static void bindConditions(CpsConcern concern)
      throws ModuleException {
