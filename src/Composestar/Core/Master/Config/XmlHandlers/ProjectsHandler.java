...
public class ProjectsHandler extends DefaultHandler implements ContentHandler 
{
	...
	public ProjectsHandler(XMLReader parser, BuildConfigHandler documentHandler)
	{
		this.parser = parser;
		...
		{	/* in <projects> */
			...
			if (buildDebugLevel != null)
			{
				try {
					...
					Debug.out(Debug.MODE_WARNING, "MASTER", "Invalid build debug level '" + runDebugLevel + "'. Expecting a number between 0 and 4. Reverting to default level 1.");
					projects.setRunDebugLevel(1);
				}
			}
