...
public class ModelGenerator
{
	...
	/**
	 * Generates a list of prolog predicates implementing the connection between
	 * Java and prolog. Static method because it doesn't need any fields.
	 * 
	 ...
	 *             stream
	 * @throws ModelException when an inconsistency is detected in the language
	 *             model
	 */
	...
	private static String genUnitTypePredicates(LanguageUnitType type)
	{
		...
		// Definition of isSomethingWithAttribute predicate (many-to-many
		// relation)
		...
		res.append("  or(is").append(typeName).append("WithName(Unit, Name), is").append(typeName).append(
				"WithNameInList(Unit,Ns)).\n\n");
	 ...
	 * Possible relations (from one unit to the other) are specified by a
	 * RelationType. 2 relationTypes make up 1 RelationPredicate (one
	 * relationType for each directions - both directions always have to exist!)
	 * 
	...
	private static String genRelationPredicate(RelationPredicate rel)
	{
		...
		else if (var1unique && !var2unique)
		{
			 ...
			 * handle on the prolog-side of the connector (which is slow!)
			 */
		}
		// else: it is a (1,1)-relation so we don't need an answer generator
		// (the answers are already unique)
	...
	/* For testing */
	/*
	 * public static void main(String[] args) { LanguageModel m =
	 * LanguageModel.instance(); try { m.createMetaModel(); PrintStream out =
	 * new PrintStream(new
	 * FileOutputStream("c:/projects/composestar/builtin.pro"));
	 * ModelGenerator.prologGenerator(m, out); out.close(); } catch (IOException
	 * e) { e.printStackTrace(); } catch (ModelException e) {
	 * e.printStackTrace(); } }
	 */
}
