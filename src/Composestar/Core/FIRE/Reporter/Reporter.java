...
public class Reporter
{
	...
	protected LinkedList stateStringList = new LinkedList(); // Remember the previous two strings.
...
//			System.out.println ("cur: " + currentStateString);
	...
	/*
	protected int getMaxRowLength()
	{
		int max = 0;

		for (int r = 0; r < rows.size(); r++)
		{
			LinkedList cr = (LinkedList) rows.get(r);
			if (cr.size() > max) max = cr.size();
		}
	...
	*/
	...
	/*
	// Not used.
	protected int [] calculateColumnLengths()
	{
		int [] colSize = new int [getMaxRowLength()];
		for (int i = 0; i < colSize.length; i++) colSize[i] = 0;
		...
		for (int r = 0; r < rows.size(); r++)
		{
			...
			for (int i = 0; i < cr.size(); i++)
			{
				...
				if (((String) cr.get(i)).length() > colSize[i]) 
				{
					colSize[i] = ((String) cr.get(i)).length();
				}
			}
		}
	...
	protected void printItem(String item, int size)
	{
		System.out.print (item);

		for (int i = 0; i < size  - item.length(); i++) System.out.print (" ");
	}
	*/
	...
	/*
	public void sortRows(int columnNr)
	{
		for (int r = 0; r < rows.size(); r++)
		{
			LinkedList cr = (LinkedList) rows.get(r);
