...
public class FireModel
{
	...
	public FireModel(Concern concern, FilterModuleOrder order)
	{
		...
		for (int i = 0; i < v.size(); i++)
		{
			...
			modules[i] = (FilterModule)ds.getObjectByID(ref);
		}
...
//			boolean flowFalse = false;
			...
			if ("inner".equals(matchTarget.getName()))
			{
				List methods;
			...
			else
			{
				...
				else
				{
					...
					if (wrapper.getRelationType() == MethodWrapper.UNKNOWN) return SIGNATURE_MATCH_UNKNOWN;
	...
	private ExtendedExecutionState deriveState(ExecutionState baseState, ExtendedExecutionState startState, int layer)
	{
		...
		else
		{
			...
			Target derivedTarget = (Message.checkEquals(newStateMessage.getTarget(), Message.STAR_TARGET)
					? message.getTarget() : newStateMessage.getTarget());

			MessageSelector derivedSelector = (Message.checkEquals(newStateMessage.getSelector(), Message.STAR_SELECTOR) 
	...
	private class ExtendedExecutionModel implements ExecutionModel
	{
		...
		{
			Set distinguishable = getDistinguishable();
			...
			while (iter.hasNext())
			{
				...
				ExtendedExecutionState extendedState = new ExtendedExecutionState(this, state, message, NO_SIGNATURE_CHECK, null, 0);

				entranceTable.put(message, extendedState);
			}

			//undistinguishable selector:
			Message message = getEntranceMessage(Message.UNDISTINGUISHABLE_SELECTOR.getName());
		...
		public ExtendedExecutionModel(String selector)
		{
			...
			ExtendedExecutionState extendedState 
				= new ExtendedExecutionState(this, state, message, NO_SIGNATURE_CHECK, null, 0);
		...
		public ExtendedExecutionModel(MethodInfo methodInfo, int signatureCheck)
		{
			...
			ExecutionState state = executionModels[0].getEntranceState(message);
			ExtendedExecutionState extendedState 
				= new ExtendedExecutionState(this, state, message, signatureCheck, methodInfo, 0);
	...
	private class ExtendedExecutionState extends ExecutionState
	{
		...
		{
			super(baseState.getFlowNode(), message, baseState.getSubstitutionSelector(), 
	...
	private class ExtendedExecutionTransition extends ExecutionTransition
	{
		...
		public ExecutionState getEndState()
		{
			if (endState == null)
			{
				int newLayer = (nextLayer ? startState.layer + 1 : startState.layer);
