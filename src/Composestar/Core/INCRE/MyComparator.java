...
public class MyComparator {
	...
	public boolean compare(Object a, Object b) throws ModuleException
	{
		...
		if(a==null && b==null)
			...
			/*if(module.equals("LOLA"))
				System.out.println("Compare: "+a+" with "+b);*/
			/*if(a instanceof FilterModuleReference)
			 {
			 System.out.println("PRINT:" + a.getClass().getName());
			 FilterModuleReference fmref = (FilterModuleReference)a;
			 fmref.getFields();
			 }*/
			...
			if(a.getClass().getName().equals(b.getClass().getName()))
			{
				...
				{
					// lets do a test to see if it is wise to implement the following
					// store hashcodes of the objects being tested
					// count the number of duplicates
					 //int key = a.hashCode();
					 //if(objects.containsKey(""+key))
					 //{
					 //System.out.println("DUPLICATE #"+duplicates+"FOUND FOR CLASS "+a.getClass().getName());
					 //duplicates++;
					 //return true;
					 //}
					 //else 
					...
					if(hasComparableObjects(a)){
						...
						//if(module.equals("LOLA"))
							//System.out.println("Compare INCRE fields "+a.getClass().getName());
					...
					else 
					{
						...
						while(enumFields.hasMoreElements())
						{
							...
							try 
							{
								// only public fields can be compared
								//System.out.println("Compare field "+field.getName());
	 ...
	 * @return boolean
	 * @param Object obj
	 * returns true if the object has an 'getINCREfields' method
	 */
	public boolean hasINCREfields(Object obj)
	{
		...
		for(int i=0; i<methods.length; i++) 
		{
			...
			//System.out.println(obj+": "+method.getName());
			if(method.getName().equals("getINCREfields")){
				System.out.println("Has incre fields!: "+c.getName());
				return true;	
			}	
		}
	...
	public boolean compareINCREfields(Object a, Object b) throws ModuleException
	{
		try 
		{
			...
			{
				//if(module.equals("LOLA"))
					//System.out.println("Comparing object for "+fullname+" [ " + compObjects.size()+" ]");
				...
				{
					//System.out.println("Already made comp "+key);
					/* Already made comparison before */
				 	duplicates++;
				 	equal = getComparison(key);
				}
				else {
					if(key!=null)/*Temporarily set result to true to avoid infinite loops*/
				...
				if(!equal)/*stop comparison by returning false as result*/					
					return false;			
			}
		}
		catch(Exception e){throw new ModuleException(e.toString(),"INCRE");}
		
		return true;
	}

	/*
	public void printFields(Object obj, int depth)
	{
		// print all fields of a object and adds depth level
		if(obj.getClass().equals(String.class))
		{
			// print string value
			System.out.println("String at level "+depth);
		}
		else if(obj.getClass().equals(Integer.class))
		{
			// print string value
			System.out.println("Integer at level "+depth);
		}
		else if(obj.getClass().equals(Boolean.class))
		{
			// print string value
			System.out.println("Boolean at level "+depth);
		}
		else if(obj.getClass().equals(Vector.class))
		{
			// print first element of vectors
			Vector v = (Vector)obj;
			System.out.println("Vector "+v.size()+" at level: "+depth);
			for(int j=0;j<v.size();j++)
				printFields((Object)v.elementAt(j),depth++);
		}
		else if(obj.getClass().equals(ConcernReference.class))
		{
			System.out.println("Found concern at level "+depth);
		}
		else if(obj.getClass().equals(DataMap.class))
		{
			// print first element of vectors
			DataMap d = (DataMap)obj;
			System.out.println("DataMap "+d.keys.size()+" at level: "+depth);
			for(int i=0;i<d.keys.size();i++)
				printFields((Object)d.values.elementAt(i),depth++);
		}
		else 
		{
			//iterate over all fields
			System.out.println(obj.getClass().getName()+" at level "+depth);
			Enumeration enumFields = getFields(obj.getClass()).elements();
			while(enumFields.hasMoreElements())
			{
				Field field = (Field)enumFields.nextElement();
				...
				try 
				{
					printFields(field.get(obj),depth+1);
				}
				catch(Exception excep)
				{
					System.out.println("INCRE:MyComparator: cannot get field: "+field.getName()+excep.toString());
					System.err.println("INCRE:MyComparator error: "+excep.getMessage()); 
				}
			}
		}	
	}*/
