	...
	private LinkedList takeOnePath(Node current, LinkedList nodes, LinkedList order, LinkedList multiOrder){
         ...
         while (true){
			...
			if (candidates.size()==1){
				/* ez a rész nem jó, ez már nem kell , a rekurziónak a két kettévált sorokat kell felhalmoznia, gy?jtenie
...
//		return order;
	...
	private void exploreCycle(LinkedList available, LinkedList used){
		...
		/* let's go back from the starting node till we reach a node that appears in the path again */
		Pair current;
		Edge e;
