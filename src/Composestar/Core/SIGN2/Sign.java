...
/**
 * Composition filters may alter the signature of a concerns. 
 * SIGN computes the full signature for all concerns using FIRE 
 * and detects if there are filters leading to ambiguous signatures.
 * 
 * @author Arjan de Roo
 */
public class Sign implements CTCommonModule
{
	...
	public void run(CommonResources resources) throws ModuleException
	{
		...
		resources.addResource("signaturesmodified", Boolean.TRUE);
	...
	private boolean checkMessage(Concern concern, FireModel fireModel, String messageSelector, HashSet distinguishable)
	{
		...
		while (dispatchStates.hasMoreElements())
		{
			...
			while (matchingparts.hasMoreElements())
			{
				...
				// meta-dispatch
				if (methods.length != 0) dispatch = true;
		...
		if (dispatch || true) return changed;
			...
			Target signatureMatchingTarget, HashSet distinguishable)
	{
		...
		{
			if (state.getTarget().getName().equals("inner")) return getInnerMethods(concern, state, distinguishable);
			else
			{
		...
		{
			if (signatureMatchingTarget.getName().equals("inner")) return getInnerMethods(concern, state,
					distinguishable);
			else return getTargetMethods(concern, state, signatureMatchingTarget, distinguishable);
		}
		// case 1:
		else
		{
	...
	private MethodInfo[] getInnerMethods(Concern concern, ExecutionState state, HashSet distinguishable)
	{
		...
		for (int i = 0; i < methods.size(); i++)
		{
			...
			if (distinguishable.contains(method.name())) continue;
			...
			if (dispTarget.name.equals("inner"))
			{
				...
				if (containsMethod(methods, targetMethod)) result.addElement(method);
			}
			else
			{
				...
				if (targetSignature.hasMethod(targetMethod)) result.addElement(method);
			}
		}
	...
	private MethodInfo[] getTargetMethods(Concern concern, ExecutionState state, Target donor, HashSet distinguishable)
	{
		...
		for (int i = 0; i < methods.size(); i++)
		{
			...
			{
				// if inner, check inner methods:
				if (containsMethod(innerMethods, targetMethod)) result.addElement(method);
			}
			else
			{
				// else check signature methods:
	...
	private void checkNonDispatchable(Concern concern, FireModel fireModel, String selector)
	{
		...
		if (signature.hasMethod(selector)) return;
		...
		while (enu.hasMoreElements())
		{
			...
			// get the dispatch selector:
			MessageSelector dispSelector = state.getSubstitutionSelector();
			if (Message.checkEquals(dispSelector, Message.STAR_SELECTOR)) dispSelector = state.getMessage()
					.getSelector();
	...
	private int resolveMethodDispatch(Concern concern, FireModel fireModel, MethodInfo methodInfo)
	{
		...
		while (enu.hasMoreElements())
		{
			...
			if (result == IN_SIGNATURE) return IN_SIGNATURE;
		}
		...
		while (enu.hasMoreElements())
		{
			state = (ExecutionState) enu.nextElement();
			int result = resolveDispatchExistence(concern, methodInfo, state);
			if (result != NOT_IN_SIGNATURE) return POSSIBLE;
		}
...
//		List methods;
		if (dispTarget.name.equals("inner"))
		{
			...
			if (m == null)
			{
				Debug.out(Debug.MODE_ERROR, MODULE_NAME, "The methodcall to method '" + methodInfoString(method)
...
//			Type type = (Type) concern.getPlatformRepresentation();
	...
	public void printConcernMethods(CommonResources resources)
	{
		...
		Iterator conIter = ds.getAllInstancesOf(Concern.class);
		...
		while (conIter.hasNext())
		{
			...
			if (st != null && concern.getDynObject("superImpInfo") != null)
			{
				...
				while (itr.hasNext())
				{
					...
					if (mw.getRelationType() == MethodWrapper.REMOVED || mw.getRelationType() == MethodWrapper.ADDED) signaturesmodified = true;
					...
					if (!Configuration.instance().getProperty("Platform").equalsIgnoreCase("c"))
					{
						...
						Debug.out(Debug.MODE_INFORMATION, "Sign", "\t\t[ " + relation + " ]  (" + returntype + ") "
