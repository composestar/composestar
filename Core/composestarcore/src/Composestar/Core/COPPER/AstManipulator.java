...
public class AstManipulator implements CpsTokenTypes {
  ...
  public boolean checkifExists(String path) {
    ...
    for (i = 0; i < ph.getNumberOfPieces(); i++) {
      ...
      if (i == 0) {
        ast = checkRoot(piece, ast);     //first item in path is always root, no need to navigate, just check
      } else {
  ...
  public boolean add(String path) {
    ...
    //   b. probeer: ast = navigateTo(deeltje@1, AST) <-- opletten of er niet 2 zijn
    //   c. lukt dat niet, dan bestaat die niet, dus aanmaken (gebruik tabel) <-- opletten of je niet moet inserten
    ...
    for (i = 0; i < ph.getNumberOfPieces(); i++) {
      piece = ph.getNextPiece();
      if (piece.isCreateNew()) {     //altijd toevoegen
        ast = addTo(piece, ast, path);
      } else {                      //try to navigate, then add
        if (i == 0) {
          ast = checkRoot(piece, ast);     //first item in path is always root, no need to navigate, just check
        } else {
          astback = ast;
  ...
  private AST addTo(Piece piece, AST ast, String totalpath) {
    ...
    if((piece.getPosition() == null) || "last".equals(piece.getPosition())) {
      ...
      ast.addChild(astnew);
    } else {        //specific position specified
      astnew = new CommonAST();
      astnew.setText(piece.getText());
      astnew = insertType(astnew, piece.getText());
  ...
  private AST searchChildren(Vector children, String text, String position, String totalpath) {
    ...
    if (text != null) {             //because we can also have @num and then the text doesn't matter
  ...
  private Vector getAllChildren(AST ast) {
    ...
    try {
      for (i = 0; i < count; i++) {
        if (i == 0) {
          curast = ast.getFirstChild();   //alleen de 1e keer getchild gebruiken
        } else {
