...
DD {
	...
	<xsl:template name="module">
		...
		<h1>
			<button onclick="var elm = document.getElementById('{$id}'); elm.style.display = (elm.style.display == 'block')?'':'block';">
				+
      </button>
			<xsl:text>
			</xsl:text>
		...
		<div class="module" id="{$id}">
			<xsl:for-each
				select="//timer[@name=$modulename]/event[not(text()=preceding::event[parent::*[@name=$modulename]]/text())]/child::text()">
				<xsl:sort data-type="text" select="." />
	...
	<xsl:template name="event">
		...
		<div class="event" id="{$id}">
			...
			<dl>
				<dt>Lowest</dt>
				<dd>
					<xsl:value-of select="floor($lowest div 1000) div 1000" />
					ms
				</dd>
				<dt>Highest</dt>
				<dd>
					<xsl:value-of select="floor($highest div 1000) div 1000" />
					ms
				</dd>
				<dt>Average</dt>
				<dd>
					<xsl:value-of select="floor($total div $eventcount div 1000) div 1000" />
					ms
				</dd>
			</dl>
