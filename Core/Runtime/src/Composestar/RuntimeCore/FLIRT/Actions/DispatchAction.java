 ...
 * DispatchAction.java 2067 2006-10-13 18:48:40Z elmuerte $ Models the action to
...
public class DispatchAction extends ComposeStarAction
{
	 ...
	 * is actually delegated to the Invoker in the until package
	 * 
			...
			else
			{
				...
				switch (m.STATE)
				{
					case Message.MESSAGE_NONSTATIC_NONSTATIC_VOID:
						MessageHandlingFacility.handleVoidMethodCall(this.continueMessage.getSender(), m.getTarget(), m
								.getSelector(), this.args);
						break;
					case Message.MESSAGE_NONSTATIC_NONSTATIC_RETURN:
						returnValue = MessageHandlingFacility.handleReturnMethodCall(this.continueMessage.getSender(),
								m.getTarget(), m.getSelector(), this.args);
						break;
