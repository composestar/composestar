...
public class AnnotationSuperImposition
{
  ...
  public void run() throws ModuleException
  {
  	...
  	INCRETimer gather = incre.getReporter().openProcess("LOLA","gatherDependencyAlgorithmInputs",INCRETimer.TYPE_NORMAL);
  ...
  /**
   * The dependency resolution algorithm needs 2 inputs:
   * - List of (predicate-based) selectors in the application
   * - List of annotation super-imposition actions
   * 
 ...
 * @throws ModuleException
		...
		for (Iterator selectorIter = selectors.iterator(); selectorIter.hasNext();)
		{
			...
			if (sel.name.equals(selDef.getQualifiedName()))
			{
				...
				while (annotsToAttach.hasNext())
				{
					...
					if (null == concernRef)
					{
						Debug.out(Debug.MODE_WARNING, "LOLA", "Annotation class " + annotRef.getQualifiedName() + " referenced in annotation binding does not exist; skipping", annotRef);
					...
					{
						Debug.out(Debug.MODE_WARNING, "LOLA", "Annotation class " + annotRef.getQualifiedName() + " referenced in annotation binding or is not a .NET class!", annotRef);
						continue; // Just skip, or should this be a fatal error?
					}
	...
	while (currentState < states.size()) // 
	{
		State myState = (State)states.elementAt(currentState);
	  	...
	  	for (int action = 0; action < annotationActions.size(); action++)
	  	{
	  		...
	  		// Restore to initial settings, i.e. only those annotations attached that existed already in the program
	  		resetAnnotationState(addedAnnots);
	  		
	  		int errorLocation;
	  		if ((errorLocation = subsetContents(myState.selectorResults, tempState.selectorResults)) != -1)
	  		{ // Don't we all loooooove descriptive error messages? At least you should be able to track the problem to a offending selector.
	  			StringBuffer msg  = new StringBuffer(512);
	  			msg.append("A negative dependency was detected between a selector and an annotation superimposition part.\n");
	  			msg.append("This is bad because the order of superimposition statements probably matters in such cases;\n");
	  		...
	  		{ // The state actually changed because of the most recent action!
	  			currentIsEndState = false; // so this is not an endstate
	  			
	  			// Is the state actually new, or is it already in the queue somewhere?
	  			...
	  			{ // It is new, so we add it at the end of the state-list
	  				...
	  				states.add(tempState); // Add this new state to the queue, to be handled later!
	  			}
	  		}
	  	}
	  	
	  	if (currentIsEndState)
	  	{ // None of the actions changed any of the selectorResults, so this is an endstate.
	  		if (null != endState && // The first endState found is always OK
	  			! equalContents(endState.selectorResults, myState.selectorResults)) // but if we find a 2nd one, it has to be equal!
	  		{ // This shouldn't happen using the current algorithm, but we didn't formally proof it so just to be sure...
				throw new ModuleException("The annotation superimposition algorithm detected that there are different possible\n "
  ...
  public Vector evaluateSelectors() throws ModuleException
  {
  	...
  	for (Iterator selectorIter = selectors.iterator(); selectorIter.hasNext();)
  	{
		...
		//System.out.println("Selector: " + selector.name + " selected [" + selector.predicate.getSelectedUnits() + "]");
	}
  	return results;
  }

  /**
   * Compares whether the contents of two vectors are equal (the contents, not just the references)
   * @param res1 a vector
  ...
  /**
   * Returns true, if every set in res1 is a subset of the corresponding set in res2
   * @param res1 vector containing sets
   * @param res2 vector containing sets (equal amount of elements in the vector as res1)
   * @return index of the first item where res1 was not a subset of res2; -1 on succes
 * @throws ModuleException
   */
  ...
  /**
   * This method attaches all the annotations that are imposed by superimposition annotation actions,
   * starting at the current action (thisAction) and working back to previous states until it reaches
   * the 'root' (initial) state.
   * @param thisState Pointer to current state
   * @param thisAction The superimposition action (index into annotationActions array) that should be applied
   *                   in this iteration. Set to -1 for 'no new action, just calculate the annotations for thisState'.
   *                   This option can be used to set the annotations according to an endstate. 
   * @return A set of all the DotNETAttributes that have been added, so they can easily be removed later
   */         
  public Set setAnnotationState(State thisState, int thisAction)
  {
  	...
  	{ // Attach the annotations specified by the current action
  		if (currAction != -1) // Skip first iteration in case of action == -1
  		{
	  		...
	  		for (Iterator progElemIter = attachTo.iterator(); progElemIter.hasNext();)
	  		{
				...
				// Currently, we don't attach the same annotation more than once.
  		...
  		currState = currState.prevState; // Will become null when we have passed the root node
  	}
  ...
  private class State
  {
  	public Vector selectorResults;  // Vector of sets - selectorResults[selectorIndex] returns set of selected program elements
  	public int lastAction; // points to element in annotationActions vector
  	public State prevState; // points to previous state; executing last_action brought us into this state
  }
