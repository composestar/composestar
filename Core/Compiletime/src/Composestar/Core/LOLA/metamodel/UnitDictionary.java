 ...
 *         queried by the prolog engine. Several indexes are created, such that
 *         looking up units should be fast in most common cases.
 */
...
public class UnitDictionary
{
	...
	// units contains all language units. The key is the classname of the unit
	// type,
	// the value is a hashset of all LanguageUnits of that type.
	private Hashtable unitsByType;

	// Used to select units by name - like units, the first level is an index
	// classType=>2nd hashtable.
	// The 2nd level hashtable contains the name of a unit as key, and the
	// object itself as value if
	// the name is guaranteed to be unique (Namespaces, Classes, Types,
	// Interfaces), or a hashset
	// of objects if they don't have to be unique (fields, operations, methods,
	// constructors).
	 ...
	 */
	/*
	 * public LanguageUnit getUnit(String id) { Iterator i =
	 * unitsByType.keySet().iterator(); while (i.hasNext()) { Hashtable list =
	 * (Hashtable) unitsByType.get(i.next()); if (list.containsKey(id)) return
	 * (LanguageUnit)list.get(id); } return null; }
	 */
	 ...
	 */
	/*
	 * public LanguageUnit getUnit(String id, String type) { if
	 * (unitsByType.containsKey(type)) return
	 * (LanguageUnit)((HashSet)unitsByType.get(type)). (id); // Result of 2nd
	 * get can still be null! return null; }
	 */
