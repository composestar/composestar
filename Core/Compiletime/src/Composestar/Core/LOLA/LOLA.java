...
public abstract class LOLA implements CTCommonModule
{
	...
	public String initLanguageModel() throws ModuleException
	{
		...
		{
			e.printStackTrace();
			Debug.out(Debug.MODE_WARNING, MODULE_NAME, "Can not write language predicates to temporary directory! Filename: " + generatedPredicatesFilename);
		}
	...
	public void createUnitIndex() throws ModuleException
	{
		...
		Debug.out(Debug.MODE_DEBUG, MODULE_NAME, "Useless information: " + registeredUnits.size() + " language units have been registered.");
		...
		try
		{
			...
			Debug.out(Debug.MODE_DEBUG, MODULE_NAME, "Useless information: " + unitDict.getAll().multiValue().size() + " language units have been kept in the dictionary.");
		}
		catch (ModelException e)
		{
			e.printStackTrace();
			Debug.out(Debug.MODE_WARNING, MODULE_NAME, "An error occurred while creating a model of the static language units");
		}
	...
	public void run(CommonResources resources) throws ModuleException
	{
		...
		if (!initialized)
		{
			INCRETimer initprolog = incre.getReporter().openProcess(MODULE_NAME,"Initialize prolog engine",INCRETimer.TYPE_NORMAL);	
		...
		if(!selectors.isEmpty()){
			...
			INCRETimer unitindex = incre.getReporter().openProcess(MODULE_NAME,"Creation of unit index",INCRETimer.TYPE_NORMAL);
			createUnitIndex();
			unitindex.stop();
			...
			INCRETimer selcalc = incre.getReporter().openProcess(MODULE_NAME,"Calculating values of selectors",INCRETimer.TYPE_NORMAL);
	...
	public ArrayList splitSelectors(ArrayList selectors) throws ModuleException{
		...
		INCRETimer step1 = incre.getReporter().openProcess(MODULE_NAME,"Split selectors based on toBeCheckedByINCRE",INCRETimer.TYPE_OVERHEAD);
		...
		INCRETimer step2 = incre.getReporter().openProcess(MODULE_NAME,"Checking query syntax",INCRETimer.TYPE_OVERHEAD);
		...
		INCRETimer step3 = incre.getReporter().openProcess(MODULE_NAME,"Checking modifications in type information",INCRETimer.TYPE_OVERHEAD);
		...
		INCRETimer step4 = incre.getReporter().openProcess(MODULE_NAME,"Checking dependencies between selectors",INCRETimer.TYPE_OVERHEAD);
