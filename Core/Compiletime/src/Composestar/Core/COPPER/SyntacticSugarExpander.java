...
public class SyntacticSugarExpander
{
	...
	public void resolveEmptySuperImposition()
	{
		...
		while (cpsIterator.hasNext())
		{
			...
			// Debug.out(Debug.MODE_DEBUG,"COPPER[EmpSuperImpResolver]","Checking
			// for valid concern: "+concern);
			if (concern != null)
			{ // We have a valid concern!
				// Debug.out(Debug.MODE_DEBUG,"COPPER[EmpSuperImpResolver]","Found
				// valid concern["+concern.getQualifiedName()+"]:
				// "+concern.getSuperImposition());
				if (concern.getSuperImposition() == null)
				{ // We do not have a superimposition part
					// Debug.out(Debug.MODE_DEBUG,"COPPER[EmpSuperImpResolver]","Found
					// valid superimp["+concern.getQualifiedName()+"]:
					// "+concern.getImplementation());
					if (concern.getImplementation() != null)
					{ // We do have an implementation part!
						// Debug.out(Debug.MODE_DEBUG,"COPPER[EmpSuperImpResolver]","Creating
						// self superimposition for:
						...
						{
							// System.out.println("Impl is of type Source:
							// "+((Source)concern.getImplementation()).getClassName());
							Iterator it = StringConverter.stringToStringList(((Source) concern.getImplementation())
									.getClassName(), ".");
							while (it.hasNext())
							{
								vtemp.add(it.next());
						...
						{
							// System.out.println("Impl is of type
							// CompiledImplementation:
							// "+((CompiledImplementation)concern.getImplementation()).getClassName());
							Iterator it = StringConverter.stringToStringList(((CompiledImplementation) concern
						...
						{
							// System.out.println("Impl is of type ???: This is
							// not supported!");
						}
