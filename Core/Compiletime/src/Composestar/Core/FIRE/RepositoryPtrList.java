...
public class RepositoryPtrList extends TreeBuilder
{
	...
	private FilterComponent parseConditionExpression(ConditionExpression ce)
	{
		...
		{
			Debug.out(Debug.MODE_WARNING,"FIRE",  "ConditionExpression is not: ConditionLiteral, And, Or, Not. What is it?");
		}
	...
	{
		// TODO: There is a bug here but I don't have any idea what to fill in :( Pascal@19-11-2004  
		//if(iterator.hasNext())
		//{
	...
	private FilterComponent parseFilter (Composestar.Core.CpsProgramRepository.CpsConcern.Filtermodules.Filter repositoryFilter)
	{
	...
	private FilterComponent parseMultipleFilters(Iterator iterator)
	{
		if (!iterator.hasNext()) 
		{
			...
			//return new Action (FilterFactory.getFilter("Dispatch"), filterNumber+1);
			return null; 
		}
	...
	private FilterComponent parseMultipleFilterModules(Iterator iterator, FilterReasoningEngine fireInfo)
	{
		...
		if (iterator.hasNext())
		{
			FilterComponent fc2 = parseMultipleFilterModules(iterator, fireInfo); // Recursion rules
			if( fc2 == null )
			{
	...
	public FilterComponent getTree (FilterReasoningEngine fireInfo)
	{
		...
		//System.out.println (rootNode.toTreeString()); no sneaky printing to the out stream!
