...
public class DoResolve
{
	...
	private void resolveFilterModuleReferences() throws ModuleException
	{
		 ...
		 * FM_AST does keeps its orignal name, but all FM (Inst) do need a
		 * unique identifier Instead of using a nice Singlton construct, I just
		...
		while (it.hasNext())
		{
			...
			{
				throw new ModuleException("FilterModuleReference '" + ref.getName()
						+ "' cannot be resolved (are you referencing a non-existent filtermodule?)", "REXREF", ref);
			}
		}
	}
	...
	// ref.setRef(temp2);
	// ref.resolved = true;
	// if (Main.debug) System.out.println("resolveCompiledConcernReprRefs:
	...
	// }
	// }
	// if(found == false) System.out.println("resolveCompiledConcernReprRefs: "
	// + ref.getName() + " not resolved");
	// }
	// }
	// }
	...
	private void resolveSelectorReferences() throws ModuleException
	{
		...
		for (it = ds.getIterator(); it.hasNext();)
		{
			...
			{ // ok, try to resolve
				...
				if (!ref.getResolved())
				{
					throw new ModuleException("SelectorReference '" + ref.getName()
							+ "' cannot be resolved (are you referencing a non-existent selector?)", "REXREF", ref);
				}
			}
		}
	}
								...
								if (temp2.getName().compareTo(ref.getFilterModule()) == 0)
								{
									...
									for (it4 = temp2.getMethodIterator(); it4.hasNext() && !found;)
									{
										...
										if ((ignoreCase) && (temp3.getName().compareToIgnoreCase(ref.getName()) == 0))
										{
											...
											// if (Main.debug)
											// System.out.println("resolveMethodReferences:
											// " + ref.getName() + "found,
											// checking parameters");
											if (compareParameters(temp3.getParameterIterator(), ref
	...
	private void resolveFilterReferences() throws ModuleException
	{
		...
		{
			obj = it.next();
			if (obj instanceof FilterReference)
			{ // ok, try to resolve
								...
								if (temp2.getName().compareTo(ref.getFilterModule()) == 0)
								{
									...
									for (it4 = temp2.getInputFilterIterator(); it4.hasNext() && !found;)
									{
										...
										if (found)
										{
											...
											ref.setResolved(true);
											// if (Main.debug)
											// System.out.println("resolveFilterReferences:
											// resolved " + ref.getName());
										}
									}
				...
				if (!ref.getResolved())
				{
					throw new ModuleException("FilterReference '" + ref.getName()
							+ "' cannot be resolved (are you referencing a non-existent filter?)", "REXREF", ref);
				}
			}
		}
	}
	...
	/**
	 * Tries to resolve all DeclaredObjectReferences (i.e. internals and
	 * externals)
	 * 
	 * @throws ModuleException
	 */
	...
	private void resolveFilterModuleParameters()
	{
		...
		while (i.hasNext())
		{
			...
			if (!pms.isList())
			{
				try
				{
					FilterModule mp = (FilterModule) ((Filter) ((FilterElement) ((MatchingPattern) ((MatchingPart) pms
	...
	/*
	 * Transforms the parameterlists of the message selector to a nested message
	 * set
	 */
	private void transformParameterList() throws ModuleException
	{
		...
		while (i.hasNext())
		{
			...
			if (pms.isList())
			{
				// we only allow this transformation on name matching, <inner.??
				// methods> is qua semantics nonsens
				if (!(((MatchingPart) pms.getParent()).getMatchType() instanceof NameMatchingType))
				{
					throw new ModuleException("Filter Module Parameter List" + pms.getName()
							+ "' cannot be transformed (<inner.??methods> is not allowed).", "REXREF");
				}
				/*
				 * on this part we change fromt he orignal idea, which was to
				 * convert the list to a nested matching, however the nesting is
				 * buggy and temporary it will be trnaformed to a full filter
				 * element.
				 */
				...
				/*
				 * we got all we want, the filter, the filterelement so the
				 * first one in the list gets the orignal FE
				 */
