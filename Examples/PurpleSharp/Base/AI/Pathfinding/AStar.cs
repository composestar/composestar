...
namespace Purple.AI.Pathfinding {
  ...
  public class AStar {
    ...
    #region Variables
    //---------------------------------------------------------------
    IPriorityQueue open;  // list of nodes
    ...
    public Move[] FindPath(IWaypoint start, IWaypoint goal, IAgent agent) {
      ...
      while (open.Count != 0) {
        Node parentNode = (Node)open.Remove();
          ...
          if (currentWaypoint.IsValid() && !closed.Contains(currentWaypoint)) {
            ...
            if (existing.Contains(currentWaypoint)) {
              ...
              if (closed.Contains(currentWaypoint)) {
                ...
                Log.Spam("This shouldn't happen for underestimating functions!");
              }
            } else {
