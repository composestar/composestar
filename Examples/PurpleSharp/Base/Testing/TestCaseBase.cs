...
namespace Purple.Testing {
    ...
    #region Initialisation
		...
		/// </summary>    
		protected void CalcMethodsToTest() {
			methodsToTest = CalcMethodsToTest(GetType());
		}   
		...
		public static IList CalcMethodsToTest(Type t) {
			IList methodsToTest = new ArrayList();

			// get all methods of this type
			MethodInfo[] methods = t.GetMethods(BindingFlags.NonPublic | 
				BindingFlags.Public    |
				BindingFlags.Instance  | 
				BindingFlags.DeclaredOnly);          
			foreach (MethodInfo m in methods) { 
				// add all methods starting with "Test"        
				if (m.Name.Substring(0,4) == "Test" && m.GetParameters().Length == 0)
					methodsToTest.Add(m);           
			}      
			return methodsToTest;
		}
