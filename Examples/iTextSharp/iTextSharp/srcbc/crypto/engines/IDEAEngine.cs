	...
	public class IDEAEngine : BlockCipher
	{
		...
		*/
			...
			int outOff)
		{
			if (workingKey == null)
			{
				throw new Exception("IDEA engine not initialised");
			}
			...
			ideaFunc(workingKey, inBytes, inOff, outBytes, outOff);
		...
		private int bytesToWord(
			...
			int     inOff)
		{
			return ((inBytes[inOff] << 8) & 0xff00) + (inBytes[inOff + 1] & 0xff);
		}
		...
		*/
			...
			int     outOff)
		{
			...
			x0 = bytesToWord(inBytes, inOff);
			x1 = bytesToWord(inBytes, inOff + 2);
			x2 = bytesToWord(inBytes, inOff + 4);
			x3 = bytesToWord(inBytes, inOff + 6);
