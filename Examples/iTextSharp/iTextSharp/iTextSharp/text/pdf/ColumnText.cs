...
//    protected ArrayList chunks = new ArrayList();
    ...
    protected ArrayList compositeElements;
    ...
    protected int GoComposite(bool simulate) {
        ...
        while (true) {
            ...
            if (element.Type == Element.PARAGRAPH) {
                ...
                int k;
                bool skipHeader = (!firstPass && table.SkipFirstHeader && listIdx <= table.HeaderRows);
                if (!skipHeader) {
                    yTemp -= table.HeaderHeight;
                    if (yTemp < minY || yTemp > maxY) {
                        if (firstPass) {
                            compositeElements.RemoveAt(0);
                            continue;
                ...
                if (listIdx < table.HeaderRows)
                    listIdx = table.HeaderRows;
                ...
                if (k < table.Size) {
                    if (table.SplitRows && (!table.SplitLate || (k == listIdx && firstPass))) {
                        if (!splittedRow) {
                            ...
                            for (int i = table.HeaderRows; i < listIdx; ++i)
                                rows[i] = null;
                        }
                        float h = yTemp - minY;
                ...
                if (!simulate) {
                    ...
                    int realHeaderRows = table.HeaderRows;
                ...
                if (!simulate) {
                    ...
                    ByteBuffer bf = gr.InternalBuffer;
    ...
    public bool ZeroHeightElement {
        get {
            return composite == true && compositeElements.Count > 0 && ((IElement)compositeElements[0]).Type == Element.GRAPHIC;
        }
    }
