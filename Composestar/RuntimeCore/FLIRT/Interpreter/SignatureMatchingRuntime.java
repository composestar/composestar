 ...
 * $Id: SignatureMatchingRuntime.java 361 2006-06-25 19:24:10Z wminnen $
 */
public class SignatureMatchingRuntime extends MatchingTypeRuntime 
{
    ...
    public boolean interpret(Message m, Dictionary context) 
	{
		...
		//if(Debug.SHOULD_DEBUG) Debug.out(Debug.MODE_DEBUG,"FLIRT","\t\t\t\tFound target: "+ct_target);
		...
		{
			if(Debug.SHOULD_DEBUG) Debug.out(Debug.MODE_INFORMATION,"FLIRT","\t\t\t\tResolving target '"+m.getInner().GetType().ToString()+"'...");
	...
	protected boolean targetHasMethod (String ct_target, String m_selector, Dictionary context)
	{
		...
		if (c == null)
		{
			...
			if(internal != null)
			{
				c = (Concern) ds.getObjectByID(internal.GetType().ToString());
				if(Debug.SHOULD_DEBUG) Debug.out(Debug.MODE_INFORMATION,"FLIRT","\t\t\t\tFound internal to resolve signature for: " + c.getName());
			}
			else
			{
				...
				if(external != null)
				{
					c = (Concern) ds.getObjectByID(external.GetType().ToString());
					if(Debug.SHOULD_DEBUG) Debug.out(Debug.MODE_INFORMATION,"FLIRT","\t\t\t\tFound external to resolve signature for: " + c.getName());
				}
			}
		}
		else
		{
			if(Debug.SHOULD_DEBUG) Debug.out(Debug.MODE_INFORMATION,"FLIRT","\t\t\t\tFound concern to resolve signature for: " + c.getName());
		}
		...
		if (c != null)  // Strange, if we got no information.
		{
			...
			if (sig != null)
			{
				if(Debug.SHOULD_DEBUG) Debug.out(Debug.MODE_INFORMATION,"FLIRT","\t\t\t\tMessage matches the signature: " + sig.hasMethod(m_selector));
