 ...
 * 
 * $Id: Column.java,v 1.1 2006/02/13 11:16:55 pascal Exp $
 * 
**/
...
public class Column implements Cloneable
{
	...
	public void addElements (int size, boolean setBoolean)
	{
		...
		for (int i = 0; i < size; i++) col[i] = setBoolean;
		colList.add(col);
	}
	...
	public void addElements (Column b)
	{
		...
		for (int i = 0; i < b.length; i++) col[i] = b.getValue(i);
		colList.add(col);
	}
	...
	public void setFalse() 			{ setAllValues(false); }
	public void setFalse(int row) 	{ setValue(row, false); }
	public void setTrue() 			{ setAllValues(true); }
	public void setTrue(int row) 	{ setValue(row, true); }

	public void setAllValues(boolean value) { setAllValues(value, 0); }
	...
	public void setAllValues(boolean value, int offset)
	{
		...
		for (int k = index; k < col.length; k++) col[k] = value;

		// and the rest.
		for (int i = table + 1; i < colList.size(); i++)
		{
			col = (boolean[]) colList.get(i);
			for (int k = 0; k < col.length; k++) col[k] = value;
		}
	}
	...
	/** Fill the internal column with a certain pattern of bits.
	 *
	 * trueBits: The block size, the number of true bits that occur
	 *           after each other. Example trueBits 2: [1 1 0 0 0 0 1 1 0 0 ] 
	 *
	 * multFalse: Multiplication of the, true bits, block size that are false and occur
	 *            after each other. Example multFalse 3: [ 1 1 0 0 0 0 0 0 1 1 ]
	 *
