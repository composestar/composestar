...
public class Sign implements CTCommonModule {
    ...
    public void run(CommonResources resources) throws ModuleException {
        Debug.out(Debug.MODE_DEBUG, MODULE_NAME,
                "Start signature generation and checking");
    ...
    private boolean checkMessage2(Concern concern, FireModel fireModel,
        ...
        execModel = fireModel.getExecutionModel(messageSelector);
        entranceState = (ExecutionState) execModel.getEntranceStates()
                .nextElement();

        CtlChecker checker = new CtlChecker(execModel, DISPATCH_FORMULA,
                dictionary);
        ...
        while (dispatchStates.hasMoreElements()) {
            ...
            CtlChecker matchingPartChecker = new CtlChecker(execModel,
                    MATCHPART_FORMULA, dictionary);
            Enumeration matchingparts = matchingPartChecker.matchingStates();

            while (matchingparts.hasMoreElements()) {
                ExecutionState matchingpartState = (ExecutionState) matchingparts
                        .nextElement();
                ...
                CtlChecker reachableChecker = new CtlChecker(execModel,
                        SIGMATCH_FORMULA, dictionary);
                earlierSignatureMatch = !reachableChecker
                        .matchesState(entranceState);

                // add methods:
                methods = getMethods(concern, messageSelector, state,
                        matchingpartState, distinguishable);
    ...
    private MethodInfo[] getMethods(Concern concern, String selector,
            ExecutionState dispatchState, ExecutionState matchingState,
            HashSet distinguishable) {
        FlowNode node = matchingState.getFlowNode();
        if (node.containsName("NameMatchingPart")) {
            return getMethods(concern, selector, dispatchState, true, null,
                    distinguishable);
        } else if (node.containsName("SignatureMatchingPart")) {
    ...
    private MethodInfo[] getMethods(Concern concern, String selector,
        ...
        // case 5 and 6
            ...
            if (signatureMatchingTarget.getName().equals("inner"))
                ...
                return getTargetMethods(concern, state,
                        signatureMatchingTarget, distinguishable);
        }
        // case 1:
        else {
    ...
    private MethodInfo[] getInnerMethods(Concern concern, ExecutionState state,
        ...
        for (int i = 0; i < methods.size(); i++) {
            ...
            if (Message.checkEquals(dispSelector,
                    Message.UNDISTINGUISHABLE_SELECTOR)) {
    ...
    private MethodInfo[] getTargetMethods(Concern concern,
            ExecutionState state, HashSet distinguishable) {
        Vector result = new Vector();
    ...
    private MethodInfo[] getTargetMethods(Concern concern,
            ExecutionState state, Target donor, HashSet distinguishable) {
        Vector result = new Vector();
    ...
    private int resolveMethodDispatch(Concern concern, FireModel fireModel,
            MethodInfo methodInfo) {
        ExecutionState state;
        ExecutionModel execModel;

        // first check with strict signature checks:
        execModel = fireModel.getExecutionModel(methodInfo,
    ...
    private void checkMethodDispatch(Concern concern, FireModel fireModel,
            MethodInfo methodInfo) {
        ExecutionState state;
        ExecutionModel execModel;
