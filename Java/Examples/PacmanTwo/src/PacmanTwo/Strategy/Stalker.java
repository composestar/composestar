 ...
 * @version $Id: Stalker.java 2663 2006-11-12 22:24:53Z elmuerte $
 */
...
public class Stalker extends Strategy {
	...
	public static int getNextMove(Pawn pawn, Level level) {
		...
		// Note: this makes the assumption a player and only a player controlls
		// a pacman
		Pacman pm = (Pacman) game.getPlayer(
				ghost.getId() % game.getPlayerCount()).getPawn();
		 ...
		 * if (gX > pX) // pacman is to the left { if
		 * (level.canMove(Direction.LEFT, gX, gY)) return Direction.LEFT; } else {
		 * if (level.canMove(Direction.RIGHT, gX, gY)) return Direction.RIGHT; }
		 * if (gY > pY) // pacman is to the top { if
		 * (level.canMove(Direction.UP, gX, gY)) return Direction.UP; } else {
		 * if (level.canMove(Direction.DOWN, gX, gY)) return Direction.DOWN; } }
		 * else // try to decrease Y dist { if (gY > pY) // pacman is to the top {
		 * if (level.canMove(Direction.UP, gX, gY)) return Direction.UP; } else {
		 * if (level.canMove(Direction.DOWN, gX, gY)) return Direction.DOWN; }
		 * if (gX > pX) // pacman is to the left { if
		 * (level.canMove(Direction.LEFT, gX, gY)) return Direction.LEFT; } else {
		 * if (level.canMove(Direction.RIGHT, gX, gY)) return Direction.RIGHT; } }
		 * return Direction.NONE; //return RandomMovement.getNextMove(pawn,
		 * level);
		 */
	}
}
