...
public class KeyboardController extends HumanController implements KeyListener {
	...
	protected int[][] schemes = {
			...
			{ java.awt.event.KeyEvent.VK_W, java.awt.event.KeyEvent.VK_S,
					java.awt.event.KeyEvent.VK_A, java.awt.event.KeyEvent.VK_D } };
	...
	public void keyPressed(KeyEvent arg0) {
		...
		if (key == schemes[scheme][0])
			direction = Direction.UP;
		else if (key == schemes[scheme][1])
			direction = Direction.DOWN;
		else if (key == schemes[scheme][2])
			direction = Direction.LEFT;
		else if (key == schemes[scheme][3])
