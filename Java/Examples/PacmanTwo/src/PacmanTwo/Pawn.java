 ...
 * @version $Id: Pawn.java 2663 2006-11-12 22:24:53Z elmuerte $
 */
package PacmanTwo;
...
public abstract class Pawn extends GameElement {
	...
	/**
	 * Set the current controller. This will also register the pawn with the
	 * controller. A pawn can only be controller by a single controller and a
	 * controller can only control a single pawn.
	 */
	...
	/**
	 * Returns 2 if a turn was made, 1 if movement in the same direction can be
	 * performed, 0 if a wall was hit,
	 */
	...
	protected void checkTouching() {
		...
		for (Enumeration e = game.getGameElements(); e.hasMoreElements(); /* nop */) {
			...
			// check touching
			if (((cXu > gXl && cXu <= gXu) || (cXl < gXu && cXl >= gXl))
					&& ((cYu > gYl && cYu <= gYu) || (cYl < gYu && cYl >= gYl))) {
				touch(ge);
			}
		}
	}
