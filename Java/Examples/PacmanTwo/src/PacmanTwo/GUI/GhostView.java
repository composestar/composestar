 ...
 * @version $Id: GhostView.java 2663 2006-11-12 22:24:53Z elmuerte $
 */
...
public class GhostView extends ImageView {
	public GhostView(Viewport inviewport) {
		...
		final String[] files = { "blinky/up.gif", "blinky/down.gif",
				"blinky/left.gif", "blinky/right.gif", "pinky/up.gif",
				"pinky/down.gif", "pinky/left.gif", "pinky/right.gif",
				"inky/up.gif", "inky/down.gif", "inky/left.gif",
				"inky/right.gif", "clyde/up.gif", "clyde/down.gif",
				"clyde/left.gif", "clyde/right.gif", "scaredghost/scared.gif",
				"scaredghost/inverted.gif" };
		getImages(files);
	}
	...
	public void render(Graphics g, GameElement ge, float delta) {
		...
		g.drawImage(img, margin + (int) Math.round(gh.getX() * MazeCell.SIZE),
				margin + (int) Math.round(gh.getY() * MazeCell.SIZE), viewport);
	}
