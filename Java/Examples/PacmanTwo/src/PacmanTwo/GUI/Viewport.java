 ...
 * @version $Id: Viewport.java 2663 2006-11-12 22:24:53Z elmuerte $
 */
...
import PacmanTwo.KeyboardController;
import PacmanTwo.MazeCell;
import PacmanTwo.Tickable;
...
public class Viewport extends Panel implements Tickable {
	...
	protected void render(float delta) {
		...
		// float buffering --
		if (bufferGraphics == null && getSize().width > 0
				&& getSize().height > 0) {
		...
		for (int a = 0; a < max; a++) {
			for (int b = 0; b < max; b++) {
				...
				if ((screendata[b][a] & MazeCell.RIGHT) != 0) {
					// right line
					g.drawLine(x + MazeCell.SIZE - 1, y, x + MazeCell.SIZE - 1,
							y + MazeCell.SIZE - 1);
				}
				if ((screendata[b][a] & MazeCell.DOWN) != 0) {
					// bottom line
					g.drawLine(x, y + MazeCell.SIZE - 1, x + MazeCell.SIZE - 1,
							y + MazeCell.SIZE - 1);
				}
