 ...
 * @version $Id: Bonus.java 2663 2006-11-12 22:24:53Z elmuerte $
 */
...
import PacmanTwo.ConcernImplementations.*;
...
public class Bonus {
	...
	public void tick(ReifiedMessage rm) {
		float delta = (Float) (rm.getArgument(0));

		rm.proceed();

		if (bonusPickups >= 2)
			return; // max of 2 pickups per level
		timeTillBonus -= delta;
	...
	public void pacmanTouch(ReifiedMessage rm) {
		GameElement ge = (GameElement) rm.getArgument(0);
		System.err.println("pacmanTouch:" + ge);
		rm.proceed();
