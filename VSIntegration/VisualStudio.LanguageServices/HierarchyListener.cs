...
namespace Composestar.StarLight.VisualStudio.LanguageServices {
    ...
    internal class HierarchyListener : IVsHierarchyEvents, IDisposable {
        ...
        public int OnItemAdded(uint itemidParent, uint itemidSiblingPrev, uint itemidAdded) {
            // Check if the item is a concern file.
            Debug.WriteLine("\n\tOnItemAdded\n");
            string name;
            if (!IsConcernFile(itemidAdded, out name)) {
                return VSConstants.S_OK;
            }
        ...
        public int OnItemDeleted(uint itemid) {
            ...
            string name;
            if (!IsConcernFile(itemid, out name)) {
                return VSConstants.S_OK;
            }
        ...
        private bool IsConcernFile(uint itemId, out string canonicalName) {
            ...
            // This item is a file; find if it is a concern file.
            hr = hierarchy.GetCanonicalName(itemId, out canonicalName);
            if (Microsoft.VisualStudio.ErrorHandler.Failed(hr)) {
                return false;
            }
            string extension = System.IO.Path.GetExtension(canonicalName);
            return (0 == string.Compare(extension, ComposeStarConstants.composeStarFileExtension, StringComparison.OrdinalIgnoreCase));
        }
        ...
        private void InternalScanHierarchy(uint itemId) {
            ...
            while (VSConstants.VSITEMID_NIL != currentItem) {
                ...
                string itemName;
                if ((null != onItemAdded) && IsConcernFile(currentItem, out itemName)) {
