...
namespace Composestar.StarLight.VisualStudio.Project
{
    ...
    /// </summary>
    [Guid("B375E800-1FF1-4dde-A9C9-B46E8E73F383")]
    public class EditorFactory : IVsEditorFactory
    {
        #region const
        internal static readonly Guid guidEditorFactory = new Guid("{B375E800-1FF1-4dde-A9C9-B46E8E73F383}");
        #endregion
        ...
        #region ctors
        public EditorFactory(ComposeStarProjectPackage package)
        {
        ...
        private IntPtr CreateFormView(IVsHierarchy hierarchy, uint itemid, IVsTextLines textLines, ref string editorCaption, ref Guid cmdUI)
        {
            ...
            try
            {
                ...
                designerLoader.Initialize(serviceProvider.GetService(typeof(IOleServiceProvider)), hierarchy, (int)itemid, textLines);
                loaderInitalized = true;

                // Create the designer
                IVSMDDesigner designer = designerService.CreateDesigner(serviceProvider.GetService(typeof(IOleServiceProvider)), designerLoader);
