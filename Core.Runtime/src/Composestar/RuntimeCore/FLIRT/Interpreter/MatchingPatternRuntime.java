 ...
 * MatchingPatternRuntime.java 2364 2006-10-30 15:18:47Z elmuerte $
 */
public class MatchingPatternRuntime extends ReferenceEntityRuntime implements Interpretable
{
	...
	public boolean interpret(MessageList m, Dictionary context)
	{
		...
		// temporary
		/*
		 * MatchingPartRuntime theMatchingPartRuntime = null; if(
		 * matchingParts.size() > 0 ) theMatchingPartRuntime =
		 * (MatchingPartRuntime) matchingParts.get( 0 ); SubstitutionPartRuntime
		 * theSubstitutionPartRuntime = null; if( substitutionParts.size() > 0 )
		 * theSubstitutionPartRuntime = (SubstitutionPartRuntime)
		 * substitutionParts.get( 0 ); if(Debug.SHOULD_DEBUG)
		 * Debug.out(Debug.MODE_INFORMATION,"FLIRT","\t\t\tInterpreting
		 * matchingPattern '"+theSubstitutionPartRuntime.getReference()+"'..." );
		 * boolean returnvalue = theMatchingPartRuntime.interpret(m,context);
		 * if(returnvalue && theSubstitutionPartRuntime != null &&
		 * theSubstitutionPartRuntime.getReference() != null) { returnvalue =
		 * theSubstitutionPartRuntime.interpret(m,context); } return
		 * returnvalue;
		 */
	...
	private boolean match(MessageList modml, Dictionary context)
	{
		...
		if (!isMessageList && (ml.getMessages().size() > 0))
		{
			for (int j = 0; j < ml.getMessages().size(); j++)
			{
				...
				{
					if (((MatchingPartRuntime) matchingParts.get(i)).interpret((Message) ml.getMessages().get(j),
							context))
					{
						ml.matchAll();
						return true;
